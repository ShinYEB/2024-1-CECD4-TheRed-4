//
//  NewScenarioPriceCollectionViewCell.swift
//  stocksignal
//
...
final class NewScenarioBuyPriceCollectionViewCell: UICollectionViewCell {
    static let id = "NewScenarioBuyPriceCollectionViewCell"
    ...
    override init(frame: CGRect) {
        ...
        downCheckButton.snp.makeConstraints { make in
            ...
            make.top.equalTo(uplabel2.snp.bottom).offset(10)
        }
        ...
        downlabel2.snp.makeConstraints { make in
            make.top.equalTo(downlabel.snp.bottom).offset(5)
    ...
    public func setLabel(viewModel: NewScenarioViewModel) {
        upInput.rx.text.orEmpty
            .subscribe(onNext: { text in
                viewModel.targetPrice_buy_price_up = Int(text) ?? 0
            })
            .disposed(by: disposeBag)
        
        downInput.rx.text.orEmpty
            .subscribe(onNext: { text in
                viewModel.targetPrice_buy_price_down = Int(text) ?? 0
            })
            .disposed(by: disposeBag)
    }
