//
//  PracticeNewScenarioViewController.swift
//  stocksignal
//
...
class PracticeNewScenarioViewController: UIViewController {
    
    private let viewModel = NewScenarioViewModel()
    private var practiceViewModel = PracticeViewModel()
    ...
    init (scenarioId: Int?, scenarioName: String, stockName: String, currentPrice: Int, quantity: Int, viewModel: PracticeViewModel) {
    ...
    private func setButton() {
        ...
        footer.confirmButton.rx.tap.bind {[weak self] in
            ...
            {
                let out = self?.viewModel.practiceMakeScenario(scenarioName: self!.scenarioName, companyName: self!.stockName, currentPrice: self!.currentPrice, buttonIndex: self!.buttonIndex, footerIndex: self!.footerIndex)
                
                if out == nil {
                    print("nil")
                }
                else {
                    self?.practiceViewModel.scenarios.append(out!)
                ...
                let out = self?.viewModel.practiceAddCondition(scenarioId: self!.scenarioId!, currentPrice: self!.currentPrice, buttonIndex: self!.buttonIndex, footerIndex: self!.footerIndex)
                
                if out == nil {
                    print("nil")
                }
                else {
                    self?.practiceViewModel.conditions.append(out!)
                    let viewController = AdaptModalViewController()
                    viewController.modalPresentationStyle = .overFullScreen
                    self?.present(viewController, animated: false)
                    DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { [weak viewController, weak self] in
